...
public class Player {
    ...
    private int picId;//玩家的立绘id（后续添加角色选择功能时候需要用到）
    private String color;//玩家的颜色
    ...
    private final Map<Integer, Color> playerColors = new HashMap<>() {{
        put(1, javafx.scene.paint.Color.RED);
        put(2, javafx.scene.paint.Color.CYAN);
        put(3, javafx.scene.paint.Color.MAGENTA);
        put(4, javafx.scene.paint.Color.ORANGE);
    }};
    ...
    public Player(int id, String name, int position, int money, boolean bankruptcy, boolean inJail, boolean isTurtle, int TurtleCount, int TurtleNum, int outJailNum, int SleepNum, boolean isSleep, int SleepCount, int ExchangeNum, int BlockNum, boolean isBlock,CardManager cards,int picId) {
        this.id = id;
        this.name = name;
